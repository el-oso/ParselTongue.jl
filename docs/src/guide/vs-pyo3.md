...
Both guarantee the GIL is released — and any temporary Python references dropped —
on every exit path, including errors. Each does so with the tool its language
provides:

- **ParselTongue** wraps the whole callback in a single `try/finally`. The GIL, the
  argument tuple, and the call result are released in the `finally`, so they are
  dropped uniformly on success, on a Python-side exception, and on a Julia-side throw
  (for example an `InexactError` while marshalling an argument). `try/finally` is a
  first-class scope-exit construct in Julia; this mirrors the
  `__attribute__((cleanup))` scope guards ParselTongue already uses in its generated
  C shim, so one cleanup block covers every path.
- **PyO3** uses Rust's ownership model: `Python::with_gil(|py| …)` releases the GIL at
  the end of the closure scope, and the `Bound<'py, PyAny>` / `Py<T>` smart pointers
  drop their references when they fall out of scope, with `PyResult<T>` and `?` for
  early returns.

Different mechanisms, the same outcome for the extension author: leak-free callbacks
with no manual bookkeeping at the call site.

ParselTongue uses `try/finally` here by design. We also prototyped Rust-style
`Result`/`Option` propagation (via
[ErrorTypes.jl](https://github.com/jakobnissen/ErrorTypes.jl)) and kept `try/finally`,
because the guarantee needed here is cleanup-on-scope-exit — which `try/finally`
expresses directly — whereas `Result` carries return-value outcomes and would still
defer the cleanup to a `finally`. Matching the construct to the task keeps the
generated body small and the safety property easy to read.
