...
| Classes | `@pyhandle` (isbits structs → real Python types; `isinstance`, auto field access, `@pymethod __repr__`/`__str__`/`__len__`/`__hash__`/`__bool__`/`__getitem__`) | `#[pyclass]` (full object protocol) |
