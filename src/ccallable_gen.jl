...
# Standard catch-block body shared by the @ccallable emitters: on a thrown
# ErrorException, set _pt_err=1 and copy the message into a malloc'd C string.
function _pt_catch_stmts()
    io = IOBuffer()
    print(io, "        local _buf::Ptr{UInt8} = Ptr{UInt8}(0)\n")
    print(io, "        unsafe_store!(_pt_err, Int32(1))\n")
    print(io, "        if _e isa ErrorException\n")
    print(io, "            local _m = _e.msg\n")
    print(io, "            if _m isa String\n")
    print(io, "                local _n::Int = sizeof(_m)\n")
    print(io, "                _buf = Ptr{UInt8}(Libc.malloc(_n + 1))\n")
    print(io, "                if _buf != C_NULL\n")
    print(io, "                    unsafe_copyto!(_buf, pointer(_m), _n)\n")
    print(io, "                    unsafe_store!(_buf, UInt8(0), _n + 1)\n")
    print(io, "                end\n")
    print(io, "            end\n")
    print(io, "        end\n")
    print(io, "        unsafe_store!(_pt_errmsg, _buf)\n")
    String(take!(io))
end
...
function _pymut_accessor_fields(T::Type)
    ...
    for i in 1:fieldcount(T)
        FT = fieldtype(T, i)
        (FT in SCALAR_BOUNDARY_TYPES || FT === String) || continue
        push!(out, (string(fieldname(T, i)), FT))
    end
    out
end

"""
    emit_ccallable_field_accessors(T::Type) -> String

Return the Julia source for the per-field getter/setter `Base.@ccallable`s of a
`@pymutable` type `T`. The getter `pt_field_get_T_f` reads `from_c(T, self).f`;
the setter `pt_field_set_T_f` assigns it on the live registry object (persisting
because `from_c` returns the object itself, not a copy).
"""
function emit_ccallable_field_accessors(T::Type)
    tname  = string(T.name.name)
    T_src  = _type_src(T)
    self_c = _type_src(c_abi_type(T))
    catch_stmts = _pt_catch_stmts()
    io = IOBuffer()
    for (fname, FT) in _pymut_accessor_fields(T)
        fc     = c_abi_type(FT)
        fc_src = _type_src(fc)
        ft_src = _type_src(FT)
        zero   = _zero_cval(fc)

        # Getter: from_c(T, self).f → carrier.
        get_sym = string("pt_field_get_", tname, "_", fname)
        print(io, string(
            "Base.@ccallable function ", get_sym,
            "(_self::", self_c, ", _pt_err::Ptr{Int32}, _pt_errmsg::Ptr{Ptr{UInt8}})::", fc_src, "\n",
            "    local _result::", fc_src, " = ", zero, "\n",
            "    try\n",
            "        _result = ParselTongue.to_c(ParselTongue.from_c(", T_src, ", _self).", fname, ")\n",
            "        unsafe_store!(_pt_err, Int32(0))\n",
            "        unsafe_store!(_pt_errmsg, Ptr{UInt8}(0))\n",
            "    catch _e\n", catch_stmts, "    end\n",
            "    return _result\n",
        ...
        # Setter: assign the field on the live object. Mutation persists.
        set_sym = string("pt_field_set_", tname, "_", fname)
        print(io, string(
            "Base.@ccallable function ", set_sym,
            "(_self::", self_c, ", _val::", fc_src, ", _pt_err::Ptr{Int32}, _pt_errmsg::Ptr{Ptr{UInt8}})::Cvoid\n",
            "    try\n",
            "        local _o::", T_src, " = ParselTongue.from_c(", T_src, ", _self)\n",
            "        _o.", fname, " = ParselTongue.from_c(", ft_src, ", _val)\n",
            "        unsafe_store!(_pt_err, Int32(0))\n",
            "        unsafe_store!(_pt_errmsg, Ptr{UInt8}(0))\n",
            "    catch _e\n", catch_stmts, "    end\n",
            "    return\n",
...
function emit_entry(exports::AbstractVector{PtExport}, user_path::AbstractString;
    ...
    for T in mutable_struct_types
        ...
        # file itself (it needs lexical access to the registry); here we only add
        # the per-field getter/setter entry points.
        println(io, emit_ccallable_field_accessors(T))
    end
