...
When `emit_pyproject=true` a minimal PEP 621 `pyproject.toml` is written next to
the `.whl` (name, version, `requires-python`, the `numpy` extra, and any
shared-runtime dependency), so the output directory is a publishable project
layout for `twine upload` / PyPI.
...
function _assemble_wheel(so::AbstractString, pkgdir::AbstractString, stage::AbstractString,
    ...
    # 6. Optionally emit a pyproject.toml alongside the wheel (item M) so the
    #    output directory is a pip-publishable project layout for twine/PyPI.
    if emit_pyproject
        # abi3 wheels run on any CPython ≥ 3.11; otherwise the floor is the build
        # interpreter's own version (the wheel tag is cpXY-specific anyway).
        req_py = abi3 ? ">=3.11" :
            ">=" * readchomp(`$python -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"`)
        pyproj = joinpath(abspath(outdir), "pyproject.toml")
        _write_pyproject(pyproj, mod, version; requires_python=req_py, runtime_req)
...
# The wheel is produced by juliac, not by a Python build backend; the build-system
# stub is informational so `pip`/`twine` recognise a valid project layout.
function _write_pyproject(path::AbstractString, mod::AbstractString, version::AbstractString;
                          requires_python::AbstractString=">=3.11",
                          runtime_req::Union{Nothing,AbstractString}=nothing)
    deps = String[]
    runtime_req !== nothing && push!(deps, runtime_req)
    io = IOBuffer()
    println(io, "# Generated by ParselTongue. The distributable is a prebuilt wheel")
    println(io, "# (compiled with juliac --trim); this file documents project metadata")
    println(io, "# for publishing workflows (twine upload / PyPI).")
    println(io, "[build-system]")
    println(io, "requires = [\"setuptools>=61\"]")
    println(io, "build-backend = \"setuptools.build_meta\"")
    println(io)
    println(io, "[project]")
    println(io, "name = ", repr(mod))
    println(io, "version = ", repr(version))
    println(io, "description = ", repr("$mod — Julia functions compiled to a native Python extension via ParselTongue"))
    println(io, "requires-python = ", repr(requires_python))
    if isempty(deps)
        println(io, "dependencies = []")
    else
        println(io, "dependencies = [", join((repr(d) for d in deps), ", "), "]")
    end
    println(io)
    println(io, "[project.optional-dependencies]")
    println(io, "numpy = [\"numpy\"]")
